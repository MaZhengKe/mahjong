...
public class AgariIndex {
    ...
    public static int[] analyse(byte[] m, byte[] p, byte[] s, byte[] z) {
        int[] n = n_zero.clone();
        for (int i = 0; i < 9; i++) {
            n[i] = m[i+1];
        }
    ...
    public static int calc_key(int[] n) {
        int p = -1;
        int x = 0;
    ...
    public static int calc_key(byte[] M,byte[] P,byte[] S,byte[] Z) {
        int p = -1;
        int x = 0;
        int pos_p = 0; // posの配列インデックス
        boolean b = false; // ひとつ前が0以外
